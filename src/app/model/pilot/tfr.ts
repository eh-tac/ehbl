...
import { getBool, getByte, getByteString, getInt, getShort } from "../hex";
...
export class TFR implements PilotData {
  ...
  public get MissionScores(): MissionScore[] {
    return this.BattleSummary.reduce(
      (carry: MissionScore[], battle: BattleSummary) =>
        carry.concat(battle.missions),
      []
    );
  }
  ...
  public get TrainingSummary(): TrainingSummary[] {
    return CraftLabels.map((craft: string, idx: number) => {
      const summary: TrainingSummary = {
        ...
        missions: this.combatCompletes[idx].map(
          (complete: boolean, mission: number) => {
